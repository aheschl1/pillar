...
//         std::fs::write(bytes_parser_path, code).expect("failed to write bytes parser script");
    ...
    async fn test_complex_peer_discovery() {
        ...
        node_a.inner.peers.write().await.insert(
            wallet_b.address,
        ...
        let peers_a = node_a.inner.peers.read().await;
        ...
        let peers_b = node_b.inner.peers.read().await;
        ...
        let peers_c = node_c.inner.peers.read().await;
        ...
        let peers_d = node_d.inner.peers.read().await;
    ...
    async fn test_start_account() {
        ...
        drop(already_b);
        let already_b = node_b.inner.broadcasted_already.read().await;
        assert!(already_b.contains(&message_to_hash.hash(&mut DefaultHash::new()).unwrap()));
    ...
    async fn inner_test_new_node_online(
        ...
        assert_eq!(ablocks[0].header.completion.as_ref().unwrap().hash, cblocks[0].header.completion.as_ref().unwrap().hash);
        assert_eq!(ablocks[1].header.completion.as_ref().unwrap().hash, cblocks[1].header.completion.as_ref().unwrap().hash);
        drop(chain_c);
    ...
    async fn test_invalid_transaction_not_included(){
        ...
        assert_eq!(already_b.len(), 1);
        drop(already_b);
        let already_a = node_a.inner.broadcasted_already.read().await;
        assert_eq!(already_a.len(), 1);
        drop(already_a);
    }

    #[tokio::test]
    async fn test_three_fully_connected(){
        let ip_address_a = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1));
        let port_a = 8000;
        let ip_address_b = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 2));
        let port_b = 8001;
        ...
        let mut miner_a = Miner::new(node_a).unwrap();
        ...
        discover_peers(&mut node_b).await.unwrap();
        discover_peers(&mut miner_a.node).await.unwrap();
        discover_peers(&mut node_c).await.unwrap();

        let peers_a = miner_a.node.inner.peers.read().await;
        assert!(peers_a.contains_key(&wallet_b.address));
        assert!(peers_a.contains_key(&wallet_c.address));
        drop(peers_a);
        ...
        // distribute a transaction from node a
        ...
        tokio::time::sleep(std::time::Duration::from_secs(10)).await; // wait for the transaction to be processed
    ...
    async fn test_block_missed(){
        ...
        let mut miner_b = Miner::new(node_b).unwrap();
        node_c.serve().await;
        miner_b.serve().await;
        node_a.serve().await;
        tokio::time::sleep(std::time::Duration::from_secs(2)).await;
        ...
        assert!(!peers_a.contains_key(&wallet_b.address));
        drop(peers_a);
        let peers_b = miner_b.node.inner.peers.read().await;
        assert!(!peers_b.contains_key(&wallet_a.address));
        drop(peers_b);
        ...
        node_a.inner.peers.write().await.insert(wallet_b.address, peer);
        discover_peers(&mut node_a).await.unwrap();
        discover_peers(&mut node_c).await.unwrap();
    ...
    async fn save_node(node: Node) -> TempDir{
        let path = tempfile::tempdir().unwrap();
        let persistence_manager = PersistenceManager::new(Some(path.path().to_path_buf()));
        persistence_manager.save_node(&node).await.unwrap();
        path
    }

    #[tokio::test]
    async fn test_sync_two_blocks_persistence() {
        let ip_address_a = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 8));
        let port_a = 6999;
        ...
        println!("Stopping node A - expect ConnetionRefused errors");
        node_a.stop().await;
        let path_a = save_node(node_a).await;
        println!("{path_a:?}");
        // pause a sec - TODO remove this after fixing the join on stoping
        tokio::time::sleep(std::time::Duration::from_secs(3)).await;
        // now, C will submit a transaction to B
        ...
        .unwrap();
        // pause a sec
        tokio::time::sleep(std::time::Duration::from_secs(10)).await; // time to settle, and give up waiting for full block
        // now both shold have 3 blocks
        let chain_b = node_b.inner.chain.lock().await;
        assert!(chain_b.as_ref().unwrap().depth == 2); // 3 blocks
        assert!(chain_b.as_ref().unwrap().blocks.len() == 3); // 3 blocks
        ...
        drop(chain_c);
        // put A back online
        let mut node_a = Node::new_load(&PersistenceManager::new(Some(path_a.path().to_path_buf()))).await.expect("Failed to load node A");
        node_a.serve().await;
        assert!(node_a.inner.state.read().await.clone() == NodeState::ChainSyncing);
        // wait for a bit
        tokio::time::sleep(std::time::Duration::from_secs(2)).await;
